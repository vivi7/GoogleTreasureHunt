...
@objc public protocol VFSideMenuDelegate {
    optional func sideMenuWillOpen()
    optional func sideMenuWillClose()
    optional func sideMenuShouldOpVFSideMenu () -> Bool
}

@objc public protocol VFSideMenuProtocol {
    var sideMenu : VFSideMenu? { get }
    func setContentViewController(contentViewController: UIViewController)
}

public enum VFSideMenuAnimation : Int {
    case None
    case Default
}

public enum VFSideMenuPosition : Int {
    case Left
    case Right
}
...
public extension UIViewController {
    ...
    public func sideMenuController () -> VFSideMenuProtocol? {
        var iteration : UIViewController? = self.parentViewController
        if (iteration == nil) {
            return topMostController()
        }
        do {
            if (iteration is VFSideMenuProtocol) {
                return iteration as? VFSideMenuProtocol
    ...
    internal func topMostController () -> VFSideMenuProtocol? {
        var topController : UIViewController? = UIApplication.sharedApplication().keyWindow?.rootViewController
        while (topController?.presentedViewController is VFSideMenuProtocol) {
            topController = topController?.presentedViewController;
        }
        
        return topController as? VFSideMenuProtocol
    }
}

public class VFSideMenu : NSObject {
    ...
    private var menuPosition:VFSideMenuPosition = .Left
    ...
    public weak var delegate : VFSideMenuDelegate?
    private var isMenuOpen : Bool = false
    
    public init(mySourceView: UIView, myMenuPosition: VFSideMenuPosition) {
        super.init()
        self.sourceView = mySourceView
        self.menuPosition = myMenuPosition
        self.setupMenuView()
    ...
    public convenience init(sourceView: UIView, myMenuTableViewController: UITableViewController, menuPosition: VFSideMenuPosition) {
        self.init(mySourceView: sourceView, myMenuPosition: menuPosition)
        self.menuTableViewController = myMenuTableViewController
    ...
    private func toggleMenu (shouldOpen: Bool) {
        if (shouldOpen && delegate?.sideMenuShouldOpVFSideMenu?() == false) {
            return;
        }
